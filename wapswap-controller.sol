...
abstract contract WAP_TOKEN is IBEP20, Ownable {
    function initReferral(address _referrer) external virtual;
    function referrer(address owner) external virtual view returns (address);
    function mintRewards(address account, uint256 amount) external virtual returns (bool);
}

contract WAPPOS is BEP20('WAPPOS Token', 'WAP_POS') {
    // The wap TOKEN!
    WAP_TOKEN public wap;
...
contract WAPController is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accWAPPerShare; // Accumulated WAPs per share, times 1e12. See below.
    }
    // The WAP TOKEN!
    WAP_TOKEN public WAP;
    // The WAP TOKEN!
    ...
    // Bonus muliplier for early WAP makers.
    ...
    // Deposited amount WAP in MasterChef
    uint256 public depositedWAP;
    // Referral commision
    ...
    function _getWAPPerBlock() internal view returns (uint256) {
        return WAPPerBlock;
    }
    
    function getWAPPerBlock() external view returns (uint256) {
        return WAPPerBlock;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external validatePoolByPid(_pid) {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accWAPPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function initPending(address sender, uint256 pending, string memory which) internal returns (uint256) {
        address referral = WAP.referrer(sender);
        uint256 refRewards = pending.mul(referralReward).div(100);
        uint256 pendingRewards = pending.sub(refRewards);
        safeWAPTransfer(sender, pendingRewards);
        if(referral != address(0)){
            safeWAPTransfer(referral, refRewards);
    ...
    function enterStaking(uint256 _amount, bool _isCompounding) external {
        ...
        if(_amount > 0) {
            ...
            user._lastInvested = block.timestamp;
            user._blockInvested = block.number;
            depositedWAP = depositedWAP.add(_amount);
        }
        user.rewardDebt = user.amount.mul(pool.accWAPPerShare).div(1e12);
        wapPos.mint(msg.sender, _amount);
    ...
    function leaveStaking(uint256 _amount) external {
        ...
        if(_amount > 0) {
            user.amount = user.amount.sub(_amount);
            pool.lpToken.safeTransfer(address(msg.sender), _amount);
            depositedWAP = depositedWAP.sub(_amount);
        }
        user.rewardDebt = user.amount.mul(pool.accWAPPerShare).div(1e12);
        wapPos.burn(msg.sender, _amount);
